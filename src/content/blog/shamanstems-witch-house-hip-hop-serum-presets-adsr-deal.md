---
title: "Shamanstems Witch House Hip Hop Serum Presets"
description: "Shamanstems' Witch House Hip Hop Serum Presets are available now on ADSR Sounds — a dark, atmospheric preset pack for Xfer Serum built specifically for..."
pubDate: "2026-04-20T04:25:34Z"
tags: ["vst-plugin", "serum", "synth", "music-production"]
affiliate: "https://www.adsrsounds.com/product/presets/shamanstems-witch-house-hip-hop-serum-presets/?utm_source=affiliate-bot&utm_medium=article&utm_campaign=shamanstems-witch-house-hip-hop-serum-presets-adsr-deal"
heroImage: "/images/shamanstems-witch-house-hip-hop-serum-presets-adsr-deal.jpg"
score: 4.90
dealPrice: "$25"
originalPrice: "$179.77"
draft: false
---
**TL;DR:** Shamanstems' Witch House Hip Hop Serum Presets are available now on ADSR Sounds — a dark, atmospheric preset pack for Xfer Serum built specifically for witch house and trap production. Check current pricing and grab the deal [directly on ADSR Sounds](https://www.adsrsounds.com/product/presets/shamanstems-witch-house-hip-hop-serum-presets/?utm_source=affiliate-bot&utm_medium=article&utm_campaign=shamanstems-witch-house-hip-hop-serum-presets-adsr-deal).

---

## What Is Shamanstems Witch House Hip Hop Serum Presets?

Shamanstems Witch House Hip Hop Serum Presets is a sound design pack for Xfer Records Serum, targeting producers who work in witch house, dark trap, lo-fi hip hop, and adjacent underground genres. The pack is focused on delivering eerie, occult-tinged textures and melodic elements that define the witch house aesthetic — think heavily reverbed leads, detuned pads, and murky bass tones. It's listed on ADSR Sounds, one of the most established marketplaces for third-party presets and samples.

---

## Key Features

- **Witch House & Dark Trap Sound Palette** — Presets are specifically dialed in for the gloomy, supernatural atmosphere characteristic of witch house and dark hip hop.
- **Built for Xfer Serum** — Requires a licensed copy of Serum; presets load directly into the synth's native browser with no additional setup.
- **Melodic & Textural Variety** — The pack covers multiple sound categories including leads, pads, basses, and atmospheric textures to support full track production.
- **Ready-to-Use Patches** — Presets are designed as drop-in starting points, letting producers build on them without deep sound design knowledge.
- **Genre-Specific Design** — Tuned for underground electronic and hip hop subgenres rather than mainstream EDM, making it a focused toolkit rather than a general-purpose pack.

---

## What Producers Are Saying

Witch house as a genre occupies a niche but dedicated corner of the production community, and Serum preset packs targeting it are relatively uncommon — which is part of the appeal here. Community discussions around witch house production tools tend to surface on [Reddit's r/WeAreTheMusicMakers](https://www.reddit.com/r/WeAreTheMusicMakers/) and [KVR Audio forums](https://www.kvraudio.com/forum/), where producers frequently discuss the scarcity of genre-specific presets for darker aesthetics. This is a niche product, so broader community reception is still building — check those communities for early impressions from users working in these genres.

---

## Who Is It For?

- **Witch House Producers** — The pack is designed around the core sonic language of the genre, making it a natural fit for anyone actively working in this space.
- **Dark Trap & Hip Hop Beatmakers** — Producers who layer sinister textures and lo-fi elements into trap beats will find the atmospheric patches immediately useful.
- **Serum Users Expanding Their Library** — Anyone already invested in Serum who wants genre-specific presets rather than general-purpose patches gets targeted value here.
- **Film & Game Audio Composers** — The dark, textural nature of the presets can translate well into horror, occult, or dystopian soundtrack work.

---

## Pricing & Deal Details

| Regular Price | Deal Price | You Save |
|---|---|---|
| Check product page | Check product page | Check product page |

Pricing on ADSR Sounds can shift with promotions and limited-time deals. ADSR runs sales and discounts regularly throughout the year, so this is worth monitoring if you're on the fence.

> **Verify current pricing on the product page — deals can change.**

[Check the current price and grab the deal on ADSR Sounds →](https://www.adsrsounds.com/product/presets/shamanstems-witch-house-hip-hop-serum-presets/?utm_source=affiliate-bot&utm_medium=article&utm_campaign=shamanstems-witch-house-hip-hop-serum-presets-adsr-deal)

---

## Alternatives at a Glance

| Name | Price | Key Difference |
|---|---|---|
| Cymatics Dark Trap Presets | Check store | Broader dark trap focus, less witch house specificity |
| Chuki Beats Serum Presets | Check store | More mainstream trap orientation, lighter aesthetic |
| Ghosthack Witch House Bundle | Check store | Sample-based approach rather than synth presets |

If you're strictly in witch house territory and already own Serum, this pack's genre specificity is its main advantage over broader trap libraries. If you need samples and loops alongside synth patches, a bundle-style pack may cover more ground.

---

## FAQ

**Q: Do I need Xfer Serum to use these presets?**
A: Yes. These are native Serum presets and require a licensed copy of Xfer Records Serum installed on your system.

**Q: Are these presets compatible with both Mac and Windows?**
A: Serum runs on both platforms, so the presets should work on either — but check the product page for any specific system requirements noted by the developer.

**Q: Is there a refund policy if the presets don't suit my workflow?**
A: Refund and return policies vary. Check ADSR Sounds' official terms on the product page before purchasing.

**Q: Can I use these presets in commercial releases?**
A: Licensing terms for preset packs typically allow commercial use of the audio output, but verify the specific license terms on the product page to be sure.

---

## Get the Deal

Witch house-specific Serum presets are rare — if this aesthetic fits your sound, it's worth a look before the deal changes. [Grab the Shamanstems Witch House Hip Hop Serum Presets on ADSR Sounds here.](https://www.adsrsounds.com/product/presets/shamanstems-witch-house-hip-hop-serum-presets/?utm_source=affiliate-bot&utm_medium=article&utm_campaign=shamanstems-witch-house-hip-hop-serum-presets-adsr-deal)

---

*This post contains affiliate links. We may earn a small commission at no extra cost to you.*
