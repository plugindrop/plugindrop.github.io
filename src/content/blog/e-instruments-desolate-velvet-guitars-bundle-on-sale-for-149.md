---
title: "e-instruments Desolate & Velvet Guitars Bundle on sale for $149 USD"
description: "e-instruments' Desolate & Velvet Guitars Bundle is currently ~~$269~~ **$149 USD** — a 44% discount. That's two premium Kontakt guitar libraries for the..."
pubDate: "2026-04-02T12:13:39Z"
tags: ["sales and promotions", "e-instruments", "guitar", "kontakt", "sale"]
affiliate: "https://www.pluginboutique.com/product/81-Bundles/58-Instrument-Bundles/16619-Desolate-Velvet-Guitars-Bundle?utm_source=affiliate-bot&utm_medium=article&utm_campaign=e-instruments-desolate-velvet-guitars-bundle-on-sale-for-149"
heroImage: "/images/e-instruments-desolate-velvet-guitars-bundle-on-sale-for-149.jpg"
score: 4.92
dealPrice: "$149"
originalPrice: "$269"
saleExpiry: "2026-04-30"
draft: false
---

---

**TL;DR:** e-instruments' Desolate & Velvet Guitars Bundle is currently ~~$269~~ **$149 USD** — a 44% discount. That's two premium Kontakt guitar libraries for the price of one. Buying them separately would run you $298, so [grab the bundle at Plugin Boutique](https://www.pluginboutique.com/product/81-Bundles/58-Instrument-Bundles/16619-Desolate-Velvet-Guitars-Bundle?utm_source=affiliate-bot&utm_medium=article&utm_campaign=e-instruments-desolate-velvet-guitars-bundle-on-sale-for-149) before the deal ends April 30, 2026.

## What Is the Desolate & Velvet Guitars Bundle?

The Desolate & Velvet Guitars Bundle combines two cinematic guitar libraries from e-instruments, a developer known for deeply sampled virtual instruments. Both libraries run on Native Instruments' free Kontakt Player, making them accessible without a full Kontakt license. Together, they cover a wide tonal range — from twangy, atmospheric tension (Desolate) to mellow warmth and emotive depth (Velvet).

## Key Features

- **Nine distinct guitars across two libraries** — Desolate includes four guitars (solid body, '60s chime, baritone, semi-hollow), while Velvet adds five more (semi-hollow, mellow baritone, shimmer guitar, non-traditional jazz semi-hollow, and amped acoustic).
- **Real amps and real spring reverb** — Every sample was recorded through two iconic vintage amps (American clean and British chime) with blendable dry/wet spring reverb.
- **Deep articulation system** — Plucks, chords, sustains, mutes, swells, feedback, harmonics, vibrato bar, and textured loops with major/minor variations, all switchable via modwheel.
- **Extensive FX and presets** — Velvet Guitars alone ships with 25 main presets and 20 FX chain presets covering compression, drive, delay, modulation, and reverb.
- **24-bit / 48 kHz sample quality** — Approximately 68+ GB combined content across both libraries, with around 62,000 individual samples in Velvet Guitars alone.
- **NKS 2 compatible** — Full integration with NI hardware controllers for hands-on sound design.

## What Producers Are Saying

These are relatively niche cinematic guitar libraries, so dedicated community threads are still developing. Early impressions on [VI-Control](https://vi-control.net/community/threads/out-now-velvet-guitars-by-e-instruments.169386/) and [Gearspace](https://gearspace.com/board/new-product-alert/1459625-e-instruments-releases-velvet-guitars.html) highlight the quality of the recordings and the organic feel of the performances. Users commonly mention the atmospheric textures and the usefulness for film scoring and ambient production. Check [KVR](https://www.kvraudio.com/product/desolate-guitars-by-e-instruments) for additional user ratings and discussion.

## Who Is It For?

- **Film and game composers** — Designed for scoring workflows that need atmospheric, evocative guitar textures without hiring a session player.
- **Ambient and cinematic producers** — The textured feedback, reversed swells, and shimmering harmonics cater directly to atmospheric sound design.
- **Singer-songwriter producers** — Velvet Guitars' mellow, fingerpicked tones and warm semi-hollow sounds suit intimate productions.
- **Sound designers** — The looped texture patches and extensive FX chains provide a solid starting point for creating evolving sonic landscapes.

## Pricing & Deal Details

| | Regular Price | Deal Price | You Save |
|---|---|---|---|
| Bundle | ~~$269~~ | **$149** | $120 (44%) |
| vs. buying separately | ~~$298~~ ($149 × 2) | **$149** | $149 (50%) |

That breaks down to **$74.50 per library** — half the individual retail price of either product.

e-instruments doesn't run frequent sales on these titles, so this bundle discount is notable. The deal runs until **April 30, 2026**.

> Verify current pricing on the product page — deals can change.

[Pick up the Desolate & Velvet Guitars Bundle at Plugin Boutique →](https://www.pluginboutique.com/product/81-Bundles/58-Instrument-Bundles/16619-Desolate-Velvet-Guitars-Bundle?utm_source=affiliate-bot&utm_medium=article&utm_campaign=e-instruments-desolate-velvet-guitars-bundle-on-sale-for-149)

## Alternatives at a Glance

| Name | Price | Key Difference |
|---|---|---|
| Orange Tree Samples Evolution Series | Varies (~$179/library) | Single-guitar deep-sampled libraries with more playing techniques per instrument |
| Heavyocity Mosaic Guitar | Check product page | Texture-focused with granular engine, less traditional playability |
| Native Instruments Session Guitarist series | ~$99–$149/library | Pattern-based approach with built-in strumming engine |

If you need pattern-based rhythm guitar with automatic strumming, the NI Session Guitarist series takes a different approach entirely. If you want granular, heavily processed textures over playable instruments, Heavyocity's offerings lean that direction. The e-instruments bundle stands out for its raw, amp-recorded authenticity across nine guitars at a competitive per-library cost.

## FAQ

**Q: Do I need the full version of Kontakt to use these libraries?**
A: No. Both Desolate Guitars and Velvet Guitars run on the free Kontakt Player. Desolate requires Kontakt 7.6.1+, and Velvet requires Kontakt Player 8.5.1 or later.

**Q: What plugin formats are supported?**
A: VST3, AU, AAX, and standalone (64-bit only) on both macOS (Intel and Apple Silicon via Rosetta 2) and Windows 10/11.

**Q: How large is the total download?**
A: Desolate Guitars is approximately 33 GB. Velvet Guitars is approximately 35 GB. Plan for roughly 68 GB total.

**Q: What is the refund policy?**
A: Check Plugin Boutique's product page for current return and refund terms.

## Get the Deal

Two premium cinematic guitar libraries, nine guitars, real amps, real reverb — ~~$298~~ **$149**. That's 50% off versus buying them individually, and the deal expires April 30, 2026.

[Get the Desolate & Velvet Guitars Bundle for $149 at Plugin Boutique →](https://www.pluginboutique.com/product/81-Bundles/58-Instrument-Bundles/16619-Desolate-Velvet-Guitars-Bundle?utm_source=affiliate-bot&utm_medium=article&utm_campaign=e-instruments-desolate-velvet-guitars-bundle-on-sale-for-149)

---

*This post contains affiliate links. We may earn a small commission at no extra cost to you.*
