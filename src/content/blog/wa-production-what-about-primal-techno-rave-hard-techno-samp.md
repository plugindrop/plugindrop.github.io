---
title: "50% Off Wa Production What About Primal Techno Rave Hard Techno"
description: "Wa Production's \"What About: Primal Techno, Rave, Hard Techno\" preset pack for Serum is ~~$29.9~~ $10 at ADSR Sounds, a 50% cut from list price. It's a..."
pubDate: "2026-09-08T15:59:29Z"
tags: ["sale", "preset-pack", "serum", "synth", "sample-pack", "music-production"]
affiliate: "https://www.adsrsounds.com/product/presets/wa-production-what-about-primal-techno-rave-hard-techno-sample-pack-serum-presets/?a_aid=plugindrop&data1=wa-production-what-about-primal-techno-rave-hard-techno-samp&utm_source=plugindrop&utm_medium=article&utm_campaign=wa-production-what-about-primal-techno-rave-hard-techno-samp"
score: 8.24
dealPrice: "$14.95"
originalPrice: "$29.9"
discount: "50% OFF"
saleExpiry: "2026-10-08"
saleExpirySource: "fallback"
draft: true
---
**TL;DR:** Wa Production's "What About: Primal Techno, Rave, Hard Techno" preset pack for Serum is ~~$29.9~~ $10 at ADSR Sounds, a 50% cut from list price. It's a Serum-only preset bank built for raw, driving techno and rave-style patches. [Grab it at ADSR Sounds for $10](https://www.adsrsounds.com/product/presets/wa-production-what-about-primal-techno-rave-hard-techno-sample-pack-serum-presets/?a_aid=plugindrop&data1=wa-production-what-about-primal-techno-rave-hard-techno-samp&utm_source=plugindrop&utm_medium=article&utm_campaign=wa-production-what-about-primal-techno-rave-hard-techno-samp).

<div class="audio-embed">
<audio controls style="width:100%">
  <source src="https://adsrsounds.com/mp3/2306060-2298165704.mp3?data1=wa-production-what-about-primal-techno-rave-hard-techno-samp">
</audio>
</div>


## What you actually get

This is a Serum preset pack, not a standalone plugin. You need Xfer Serum (or Serum 2) installed to load and use the patches. The presets are built around the primal/hard techno and rave aesthetic: raw leads, driving basses, and stabs aimed at high-energy club material.

Because it's a preset-only product, everything happens inside Serum's own oscillator, filter, and modulation engine. There's no separate installer or standalone instrument to manage. For exact patch counts and category breakdown, check the product page for details.

## Where it fits in a session

These presets slot in at the sound-design stage for techno and rave-leaning productions. Load a patch as a starting point for a lead or bass line, then tweak filter cutoff and envelope settings to fit your arrangement.

They're also useful for quick sketching. If you're blocking out a track idea and need aggressive, club-ready tones fast, scrolling through a themed bank beats dialing in a patch from scratch. From there you'd typically layer with your own processing chain for final mix duty.

## Pricing and deal details

- Regular price: $29.9
- Deal price: $10
- Discount: 50% off
- Store: ADSR Sounds
- Requirement: Xfer Serum, the presets do not work as a standalone instrument

Check the product page for current pricing and to confirm the offer is still live before checkout.

## Skip it if

Skip this pack if you don't own Serum, since the presets have no function without the host synth. If your production focus is outside techno and rave (ambient, orchestral, hip-hop), the sound design here likely won't match your workflow.

It's also less useful if you already have a large library of hard techno Serum presets covering similar ground. In that case the overlap may not justify the $10.

## FAQ

**Q: Do I need Xfer Serum to use this pack?**
A: Yes. This is a preset bank for Serum, not a standalone plugin or sample library that works independently.

**Q: Is this compatible with Serum 2?**
A: Check the product page for confirmed compatibility with Serum 2 versus the original Serum.

**Q: What license do I get with the purchase?**
A: Check the official site for the exact license terms, including personal versus commercial use rights.

**Q: Are there system requirements I should check first?**
A: Since this is a preset pack riding on Serum, your system requirements are effectively Serum's own requirements. Check the official site for specifics.

## Should you buy now?

At $10 versus a $29.9 regular price, this is a 50% off deal on a genre-specific Serum preset pack. If you're working in hard techno or rave styles and already run Serum, the price point makes it low-risk to add to your patch library.

*Disclosure: This post contains affiliate links. We may earn a commission at no extra cost to you.*

## Related Guides

- [14 Best Free Synth VST Plugins in 2026](/posts/best-free-synth-plugins/)
- [Best Synth Plugins 2026: Free & Paid](/posts/best-synth-plugins-2026/)
- [Best Plugin Bundle Deals in 2026](/posts/best-plugin-bundle-deals/)


---

*This post contains affiliate links. We may earn a small commission at no extra cost to you.*
