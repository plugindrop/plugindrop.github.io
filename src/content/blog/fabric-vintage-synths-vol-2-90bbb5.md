---
title: "60% Off Fabric Vintage Synths Vol 2 — Vintage Synth Sounds ($119)"
description: "Fabric Vintage Synths Vol 2 is $299 $119 at Plugin Boutique right now, a 60% cut off the regular price. It's a sample/instrument bundle built..."
pubDate: "2026-08-23T14:50:28Z"
tags: ["sale", "bundle", "effects", "synth", "music-production"]
affiliate: "https://www.pluginboutique.com/search?q=Fabric%20Vintage%20Synths%20Vol%202&a_aid=69cb95abe1763&utm_source=plugindrop&utm_medium=article&utm_campaign=fabric-vintage-synths-vol-2-90bbb5&chan=art&data1=fabric-vintage-synths-vol-2-90bbb5"
heroImage: "/images/fabric-vintage-synths-vol-2-90bbb5.jpg"
score: 9.31
dealPrice: "$119.00"
originalPrice: "$299.00"
discount: "60% OFF"
draft: false
saleExpiry: "2026-09-22"
saleExpirySource: "fallback"
---
**TL;DR:** Fabric Vintage Synths Vol 2 is ~~$299~~ **$119** at Plugin Boutique right now, a 60% cut off the regular price. It's a sample/instrument bundle built around vintage synthesizer tones for producers who want analog-style character without buying hardware. [Grab the deal at Plugin Boutique](https://www.pluginboutique.com/product/81-Bundles/58-Instrument-Bundles/17701-Fabric-Vintage-Synths-Vol-2?a_aid=69cb95abe1763&utm_source=plugindrop&utm_medium=article&utm_campaign=fabric-vintage-synths-vol-2-90bbb5&chan=art&data1=fabric-vintage-synths-vol-2-90bbb5).

## Price Context (tracked by PluginDrop)
- Regular: $299

<div class="video-embed">
<iframe loading="lazy" src="https://www.youtube-nocookie.com/embed/MI3RuYqlrFo" title="60% Off Fabric Vintage Synths Vol 2 — Vintage Synth Sounds ($119) — sound demo" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
</div>

## What Is Fabric Vintage Synths Vol 2?

Fabric Vintage Synths Vol 2 is an instrument bundle sold through Plugin Boutique, built around sampled and synthesized vintage synth tones. It's positioned as a content pack for producers who want classic analog-style textures — pads, leads, basses, and sequences — inside a modern workflow. Check the product page for the exact host format and playback engine requirements before buying.

## Key Features

- **Vintage synth sound sources** — the pack draws on classic synthesizer tones as its foundation, giving you analog-style character without owning the original hardware.
- **Ready-to-use patches** — presets are organized for quick browsing so you can audition and drop sounds into a track without deep sound design.
- **Bundle format** — this is Volume 2, suggesting it extends or complements a first volume for producers building out a full vintage synth library.
- **Genre-flexible tones** — vintage synth textures like these typically suit synthwave, electronic, pop, and hip-hop production.

## Who Is It For?

- **Producers chasing analog character** — anyone who wants warm, vintage-style synth tones without sourcing or maintaining real hardware.
- **Genre producers in synthwave, pop, or electronic** — vintage synth pads and leads are a staple texture in these styles.
- **Sound design hobbyists on a budget** — a discounted bundle is a lower-risk way to expand a sound library.
- **Owners of Vol 1 looking to expand** — if you already have the first volume, this fills out the same sonic palette.

## Pricing & Deal Details

| Regular Price | Deal Price | You Save |
|---|---|---|
| ~~$299~~ | **$119** | **$180 (60%)** |

A 60% discount is a substantial cut for an instrument bundle at this price tier.

pluginboutique.com/search?q=Fabric+Vintage+Synths+Vol+2&a_aid=69cb95abe1763&utm_source=plugindrop&utm_medium=article&utm_campaign=fabric-vintage-synths-vol-2-90bbb5).

## Alternatives at a Glance

| Name | Price | Key Difference |
|---|---|---|
| Fabric Vintage Synths Vol 2 | $119 (was $299) | Bundle format focused specifically on vintage synth tones |
| Native Instruments Kontakt libraries (vintage synth packs) | Varies, often $50-$150 | Requires the Kontakt player or full version to load |
| Arturia V Collection | Subscription or ~$300+ perpetual | Full synth emulations you can tweak in real time, not just preset content |

If you want a fixed-cost preset library you can drop into a track fast, Fabric Vintage Synths Vol 2 fits that use case. If you want to sculpt your own patches from the ground up, a full synth emulation suite like Arturia V Collection covers more ground but costs more and has a steeper learning curve.

## FAQ

**Q: Does Fabric Vintage Synths Vol 2 require a specific plugin format?**

**Q: Is this a one-time purchase or subscription?**
A: Plugin Boutique bundles are typically sold as one-time purchases with perpetual use, but confirm the license type on the product page before buying.

**Q: Do I need iLok or another authorization system?**

**Q: What's the refund policy if it doesn't work with my setup?**
A: Check Plugin Boutique's official refund policy on their site before purchasing.

## Get the Deal

$180 off a vintage synth bundle is a meaningful discount for anyone building out an analog-style sound library. The deal is live now at Plugin Boutique.

[Get Fabric Vintage Synths Vol 2 at Plugin Boutique](https://www.pluginboutique.com/product/81-Bundles/58-Instrument-Bundles/17701-Fabric-Vintage-Synths-Vol-2?a_aid=69cb95abe1763&utm_source=plugindrop&utm_medium=article&utm_campaign=fabric-vintage-synths-vol-2-90bbb5&chan=art&data1=fabric-vintage-synths-vol-2-90bbb5).

## Is This Deal Worth It?

A 60% discount is a large cut for a bundle like this, and it's worth acting on if the vintage synth tone palette fits your genre. Plugin Boutique runs frequent site-wide and vendor-specific promotions, so similar bundle discounts do resurface, but a 60% markdown on a specific title isn't guaranteed to repeat on the same schedule. If the sound is what you need, buying during a confirmed 60% window is a reasonable time to act rather than waiting on an uncertain future sale.

## Related Guides

- [14 Best Free Synth VST Plugins in 2026](/posts/best-free-synth-plugins/)
- [Best Synth Plugins 2026: Free & Paid](/posts/best-synth-vst-plugins-2026/)
- [Best Plugin Bundle Deals in 2026](/posts/best-plugin-bundle-deals/)

---

*This post contains affiliate links. We may earn a small commission at no extra cost to you.*
